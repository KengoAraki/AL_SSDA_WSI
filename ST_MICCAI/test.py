...
def main_trg(trg_l_wsi: str, config_path: str, test_set: str = "trg_unl", trg_l_set: str = "top"):
    """
    ...
    logging.basicConfig(
        level=logging.INFO,
        filename=f"{config['test']['output_dir']}st1_{config['main']['src_facility']}_{trg_l_set}_{trg_l_wsi}_{test_set}-{config['main']['trg_facility']}.txt",
    ...
    for cv_num in range(config['main']['cv']):
        ...
        project_prefix = f"st1_{config['main']['src_facility']}_{trg_l_set}_{trg_l_wsi}_cv{cv_num}_"
