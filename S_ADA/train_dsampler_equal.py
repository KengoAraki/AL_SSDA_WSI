...
def train_net(
    ...
    n_train = min(len(l_src_train_loader), len(d_l_src_train_loader), len(d_unl_trg_train_loader))
    ...
    for epoch in range(epochs):
        ...
        with tqdm(
            ...
            # 短いdataloaderに合わせる
            for l_src_batch, d_l_src_batch, d_unl_trg_batch in zip(
                l_src_train_loader, d_l_src_train_loader, d_unl_trg_train_loader
            ):
                # unl_batchのbatchサイズはl_batchの2倍
                l_src_imgs, l_src_labels = l_src_batch["image"], l_src_batch["label"]
                unl_trg_imgs, unl_trg_labels = d_unl_trg_batch["image"], d_unl_trg_batch["label"]
                d_l_src_imgs = d_l_src_batch["image"]
                d_unl_trg_imgs = d_unl_trg_batch["image"]

                # classification用
                clf_imgs = l_src_imgs.to(device=device, dtype=torch.float32)
                clf_labels = l_src_labels.to(device=device, dtype=torch.long)
                ...
                # Adversarial loss用
                adv_labels = \
                    torch.full((d_unl_trg_imgs.data.size()[0],), D_src_label, dtype=torch.long, device=device)

                # Discriminator用
                D_src_imgs = d_l_src_imgs.to(device=device, dtype=torch.float32)
                D_trg_imgs = d_unl_trg_imgs.to(device=device, dtype=torch.float32)
                D_src_labels = \
        ...
        clf_unl_trg_loss, d_unl_trg_loss, clf_unl_trg_cm, d_unl_trg_cm = \
            eval_net_trg_val(netE, netD, d_unl_trg_train_loader, criterion, device, trg_label=D_trg_label)
...
def main(config_path: str):
    ...
    for cv_num in range(config["main"]["cv"]):
        ...
        l_src_train_data, unl_trg_train_data, valid_data, test_data = dataset.get()
        ...
        checkpoint_dir = (
            ...
            + f"{config['main']['prefix']}_ADA_{config['main']['src_facility']}_{config['main']['classes']}/")
        try:
