...
def eval_net_trg_val_d(netE, netD, src_loader, trg_loader, criterion, device, src_label=0, trg_label=1):
    netE.eval()
    netD.eval()

    # n_val = len(src_loader)  # the number of batch
    n_val = min(len(src_loader), len(trg_loader))

    total_d_loss = 0

    with tqdm(total=n_val, desc="Validation round", unit="batch", leave=False) as pbar:
        for i, (src_batch, trg_batch) in enumerate(zip(src_loader, trg_loader)):
            src_imgs = src_batch['image'].to(device=device, dtype=torch.float32)
            trg_imgs = trg_batch['image'].to(device=device, dtype=torch.float32)

            d_src_labels = torch.full((src_imgs.data.size()[0],), src_label,
                                      dtype=torch.long, device=device)
            d_trg_labels = torch.full((trg_imgs.data.size()[0],), trg_label,
                                      dtype=torch.long, device=device)

            imgs = torch.cat((src_imgs, trg_imgs), 0)
            d_labels = torch.cat((d_src_labels, d_trg_labels), 0)
            ...
            # confusion matrix
            if i == 0:
                d_cm = make_confusion_matrix(d_preds, d_labels, cm=None)
            else:
                d_cm = make_confusion_matrix(d_preds, d_labels, cm=d_cm)
