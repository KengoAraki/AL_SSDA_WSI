...
from model import Discriminator, Discriminator2, Encoder
from dataset import Source_Dataset, Target_Dataset
from eval import eval_net_train_d, eval_net_test_d, tensorboard_logging
...
def train():
    ...
    # ===== set save weight dir ==== #
    save_D_weight_path = Path(f"{result_dir}/checkpoints/Discriminator2(pretrained)/best.pth")
    ...
    project = f"ADA(D2_pretrain)_src-{source}_trg-{target}_{encoder_name}_cl{classes}_cv{cv_num}"
    ...
    netE = Encoder(
        encoder_name, num_classes, pretrained=True, weight_path=weight_path_E, device=device).to(device)
    # netE = Encoder(
    #     encoder_name, num_classes, pretrained=True, weight_path=None, device=device).to(device)  # ImageNet

    # netD = Discriminator(encoder_name, num_classes).to(device)
    netD = Discriminator2(encoder_name, num_classes).to(device)
    ...
    for epoch in range(epochs):
        ...
        for i, (source_data, target_data) in enumerate(zip(train_source_loader, train_target_loader)):
            ...
            target_featuremap = target_featuremap.detach()  # stop updating Encoder(CNN)
        ...
        logging.info(f"epoch: {epoch}/{epochs}, loss_D(test): {loss_D_value_t}")
        # Recall(平均) でearly stopping?
        torch.save(
            netD.state_dict(),
            str(
