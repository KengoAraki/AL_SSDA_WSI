...
def train_net(
    ...
    for epoch in range(epochs):
        ...
        # To load max iter
        clf_l_src_train_iter = ForeverDataIterator(clf_l_src_train_loader)
        clf_l_trg_train_iter = ForeverDataIterator(clf_l_trg_train_loader)
        d_l_src_train_iter = ForeverDataIterator(d_l_src_train_loader)
        d_unl_trg_train_iter = ForeverDataIterator(d_unl_trg_train_loader)

        # l_srcの最大iteration数をmax_iterに設定
        max_iter = len(clf_l_src_train_loader)

        with tqdm(
            total=max_iter, desc=f"Epoch {epoch + 1}/{epochs}", unit="batch"
        ) as pbar:
            count_src_cluster_ids = []  # to check imblanaced sampler
            count_trg_cluster_ids = []  # to check imblanaced sampler

            for i in range(max_iter):
                clf_l_src_batch = next(clf_l_src_train_iter)
                clf_l_trg_batch = next(clf_l_trg_train_iter)
                d_l_src_batch = next(d_l_src_train_iter)
                d_unl_trg_batch = next(d_unl_trg_train_iter)
        ...
        EC_loss_value /= max_iter
        A_loss_value /= max_iter
        D_loss_value /= max_iter
