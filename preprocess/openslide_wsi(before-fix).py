...
class OpenSlideWSI(openslide.OpenSlide):
    ...
    # 条件を満たさなければNoneを返す
    def _get_output_trg_normal_dir(self, s_p, output_main_dir, obj_name, th=1):
        output_dir = None
        s_p_mode = stats.mode(
            s_p, axis=None
        )  # s_p_mode[0]:s_pの最頻値, s_p_mode[1]:s_pの最頻値の個数
        ...
        if int(s_p_mode[0]) == 255:
            # semanticパッチのクラス最頻値のピクセル数の割合と閾値を比較
            if float(s_p_mode[1] / (s_p.shape[0] * s_p.shape[1])) >= th:
                output_dir = f"{output_main_dir}0/{obj_name}/"
                os.makedirs(output_dir) if os.path.isdir(output_dir) is False else None
        else:
            # semanticパッチのクラス最頻値のピクセル数の割合と閾値を比較
            if float(s_p_mode[1] / (s_p.shape[0] * s_p.shape[1])) >= th:
    ...
    def _getBoundingBox(self, lower_obj_size=10000, test_dir=None):
            ...
            for row in range(row_max):
                ...
                if (
                    ...
                    if (
                        ...
                        output_dir = self._get_output_trg_normal_dir(
                            s_p, output_main_dir, obj_name)
...
def main():
    ...
    for wsi_path in wsi_list:
        ...
        bb_list = wsi._getBoundingBox(lower_obj_size=LOWER_OBJ_SIZE)
