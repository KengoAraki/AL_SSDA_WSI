...
from ST_ADDA2.dataset import WSIDataset_ST1_ADDA2_ValT
...
def train_net(
    ...
    """
    netSrcE: Encoder network for Source (nn.Module),
    netTrgE: Encoder network for Target (nn.Module),
    ...
    optE = select_optim(optim_name, netTrgE.parameters())
    ...
    n_train = min(len(clf_l_trg_train_loader), len(d_l_src_train_loader), len(d_unl_trg_train_loader))
    ...
    for epoch in range(epochs):
        ...
        # netTrgEのClassifierは勾配計算をしない
        for param in netTrgE.fc.parameters():
            param.requires_grad = False

        # netSrcEは勾配計算しない　
        for param in netSrcE.parameters():
            param.requires_grad = False
        ...
        with tqdm(
            ...
            # 短いdataloaderに合わせる
            for clf_l_trg_batch, d_l_src_batch, d_unl_trg_batch in zip(
                clf_l_trg_train_loader, d_l_src_train_loader, d_unl_trg_train_loader
            ):
                ...
                # classification用
                clf_imgs = clf_trg_imgs.to(device=device, dtype=torch.float32)  # class-balanced
                clf_labels = clf_trg_labels.to(device=device, dtype=torch.long)
                ...
                clf_pred = netTrgE(clf_imgs, mode="class")
                ...
                # train with source
                D_src_featuremap = netSrcE(D_src_imgs, mode="feature")
        ...
        clf_val_loss, d_val_loss, clf_val_cm, d_val_cm = \
            eval_net_trg_val(netTrgE, netD, val_loader, criterion, device, trg_label=D_trg_label)
        ...
        clf_unl_trg_test_loss, d_unl_trg_test_loss, clf_unl_trg_test_cm, d_unl_trg_test_cm = \
            eval_net_trg_val(netTrgE, netD, unl_trg_test_loader, criterion, device, trg_label=D_trg_label)
...
def main(config_path: str):
    ...
    for cv_num in range(config["main"]["cv"]):
        for l_trg_num, l_trg_selected_wsi in enumerate(l_trg_wsis):
            ...
            dataset = WSIDataset_ST1_ADDA2_ValT(
            ...
            _, clf_l_trg_data, d_src_data, d_trg_data, valid_data, test_data = dataset.get()
...
if __name__ == "__main__":
    ...
    config_path = "../ST_ADDA2/config_st-adda2_cl[0, 1, 2]_valt3.yaml"
    # config_path = "./ST_ADDA2/config_st-adda2_cl[0, 1, 2]_valt3.yaml"
